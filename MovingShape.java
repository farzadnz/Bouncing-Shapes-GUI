...
public abstract class MovingShape {
    ...
    public class SquarePath extends MovingPath {
    	...
    	int origX = topLeft.x;
    	int origY = topLeft.y;
    	boolean hasRotatedX = false;
    	boolean hasRotatedY = false;
    	
    	
    	public SquarePath(int dx, int dy) {
            deltaX = dx;
            deltaY = dy;
         }
    	
    	public void move() {
    		if(topLeft.y >= marginHeight) {
    			topLeft.y = 0 - height;
    			hasRotatedY=true;
    		} 
    		
    		else if(topLeft.y <= 0 - height) {
    			topLeft.y = marginHeight;
    			hasRotatedY=false;
    		}
    		
    		if(topLeft.x >= marginWidth) {
    			topLeft.x = 0 - width;
    			hasRotatedX=true;
    		}
    		
    		else if(topLeft.x <= 0 - width) {
    		...
    		if (direction == 0) {
    			...
    			if (hasRotatedX && topLeft.x >= maxValue - (marginWidth - origX)) {
					direction = 1;	
    			} 
    			else {
    				if (topLeft.x >= origX + maxValue) {
    					direction = 1;	
    				}
    			}
    			...
    			if (hasRotatedY && topLeft.y >= maxValue - (marginHeight - origY)) {
    				direction = 2;
    			} 
    			else {
    				if (topLeft.y >= origY + maxValue) {
        				direction = 2;
        			}
    			}
    		}
    			...
    			if (!hasRotatedX && topLeft.x <= origX) {
    				direction = 3;
    			}
    			...
    			if (!hasRotatedY && topLeft.y <= origY) {
    				direction = 0;
    			}
    		}
